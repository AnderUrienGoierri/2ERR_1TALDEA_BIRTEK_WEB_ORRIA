...
- **`SalmentaLangilea`**: Salmenta eta fakturazio logika.
  ...
  - `eskaeraOsoaSortu(Eskaera e, List<EskaeraLerroa> lerroak)`: [BERRIA] Eskaera berria eta bere lerroak transakzio bakarrean eta modu seguruan sortu.
  - `eskaeraOsoaEditatu(Eskaera e, List<EskaeraLerroa> lerroak)`: [BERRIA] Eskaera baten datuak eta bere lerroak (ezabatu, berritu) modu seguruan eguneratu.
  - `eskaeraEzabatu(int idEskaera)`: Eskaera bat eta bere lerro guztiak DBtik kendu.
...
## Web Kudeaketa Eguneraketak

Azken bertsioan, web aplikazioaren kudeaketa hobetu da bi puntu nagusitan:

### 1. Hornitzaileen Sarrera Fluxu Berria (Staging)

Hornitzaileek produktu berriak sartzean, datuak ez dira zuzenean `produktuak` taulan txertatzen. Horren ordez:

- **`sarrera_lerroak`** taulan gordetzen dira JSON formatuan (`produktu_berria_datuak` zutabea).
- Sarreraren egoera **'Bidean'** gisa markatzen da.
- Produktu hauek ez dira katalogoan agertuko langile batek sarrera **'Jasota'** gisa markatu arte (eta produktuak sortu arte).

### 2. Erosketa Murrizketak (Salgai Kontrola)

Produktuen xehetasun orrian (`produktua_xehetasunak.php`), **`salgai`** zutabearen balioa egiaztatzen da:

- `salgai = 0` bada: Erabiltzaileari "Ez dago salgai" mezua erakusten zaio eta saskiratzeko botoia desgaitzen da.
- `salgai = 1` eta `stock > 0` bada: Erosketa prozesua normal jarrai daiteke.

### 3. Irudien Kudeaketa

Irudien bideak dinamikoki kudeatzen dira orain, `http` bideak eta erlatiboak (`../`) ondo bereiziz, 404 erroreak saihesteko.
